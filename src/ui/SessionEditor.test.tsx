...
describe("SessionEditor", () => {
  ...
  describe("calendar", () => {
    ...
    it("allows adding any month if there are no existing months", async () => {
      ...
      await user.selectOptions(
        screen.getByLabelText("Add Month:"),
        "September"
      );
      await user.keyboard("{enter}");
    ...
    it("allows removing calendar items from the end of the month's list", async () => {
      ...
      expect(
        screen.queryByText("Test Calendar Item 2")
      ).not.toBeInTheDocument();
    });
    it("allows removing calendar items from the middle of the month's list", async () => {
      ...
      rerender(<SessionEditor session={sessionStore.session} />);
      expect(
        screen.queryByText("Test Calendar Item 1")
      ).not.toBeInTheDocument();
    ...
    it("allows marking a calendar item as not completed", async () => {
      ...
      fireEvent.click(screen.getByRole("button", { name: "Stop Editing" }));
      rerender(<SessionEditor session={sessionStore.session} />);
      expect(screen.getByText("Test Calendar Item")).not.toHaveClass(
        "completed"
      );
    });
    ...
    it("allows collapsing the calendar", () => {
      ...
      fireEvent.click(
        screen.getByRole("button", { name: "Collapse calendar" })
      );
  ...
  describe("topics", () => {
    it("shows the list of topics", () => {
      ...
      expect(screen.getAllByText("Test Topic", { exact: false })).toHaveLength(
        2
      );
      sessionStore.addTopic({
        title: "Test Topic 3",
