...
describe('editor', () => {
	...
	it('allows undoing changes', async () => {
		...
		const { rerender } = render(<SessionEditor session={sessionStore.session} />);
		...
		await user.type(screen.getByLabelText('Name to add to Members in attendance'), 'Bob Jones');
		await user.keyboard('{enter}');
		await user.type(screen.getByLabelText('Name to add to Members in attendance'), 'Tom Smith');
		await user.keyboard('{enter}');
		...
		expect(screen.getByText(getByTextContent('Lead by Mr. Jones'))).toBeInTheDocument();
		...
		expect(screen.queryByText(getByTextContent('Lead by Mr. Jones'))).not.toBeInTheDocument();
